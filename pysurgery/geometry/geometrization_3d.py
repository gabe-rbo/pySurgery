    ...
    def vertex_link(
        ...
        *,
        ...
        coords = np.concatenate(
            [triangle_coords, np.zeros((n_tet, 3), dtype=np.int64)], axis=1
        ).ravel()
    ...
    def edge_link(
        ...
        *,
        ...
        for t_idx, tet in enumerate(manifold.tetrahedra):
            ...
            support.append(t_idx)
        coords = np.concatenate([triangle_coords, quad_coords], axis=1).ravel()
        residual = _normal_surface_matching_residual(
            manifold,
            coords,
            matching_matrix=matching_matrix,
...
def _piece_intersects_face(
    local_vertex_count: np.ndarray,
    local_quad_count: np.ndarray,
    face_local_vertices: tuple[int, int, int],
    ...
    Args:
        ...
        is_boundary: Whether the face is on the manifold boundary.
...
def analyze_geometrization(
    ...
    *,
    ...
    if sphere_like and pi1_descriptor == "1":
        ...
        elif tri.is_closed and tri.euler_characteristic == 0:
        ...
        elif not tri.is_closed:
            classification = "manifold_with_boundary"
            status = "inconclusive"
            missing_data.append("Boundary component classification")
        else:
            classification = "hyperbolic_candidate"
        ...
        status = "success" if (allow_approx and status != "inconclusive") else status
        exact = False
        validated = False
