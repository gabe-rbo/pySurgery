...
def extract_stiefel_whitney_tangent(sc: SimplicialComplex, k: int, backend: str = "auto") -> np.ndarray:
    ...
    if k == 1:
        ...
        if rank_n == 0:  # Non-orientable
            return _w1_cocycle(sc)

        return np.zeros(sc.count_simplices(1), dtype=np.int64)

    # Fast-path for top class w^n (Euler mod 2)
    if k == n:
        # Return a representative cocycle of the top class.
        # Evaluation on the fundamental class [M] gives chi(M) mod 2.
...
def extract_stiefel_whitney_top_pairing(sc: SimplicialComplex) -> int:
    """Return the evaluation of the top Stiefel-Whitney class on the fundamental class.

    What is Being Computed?:
        The pairing ⟨w_n(TM), [M]⟩, which by Stiefel's theorem is equal to
        the Euler characteristic χ(M) mod 2.

    Args:
        sc: The simplicial complex representing a homology manifold.

    Returns:
        int: χ(M) mod 2.
    """
    return int(sc.euler_characteristic() % 2)


def _w1_cocycle(sc: SimplicialComplex) -> np.ndarray:
    """Compute the first Stiefel-Whitney cocycle w1.

    What is Being Computed?:
        A representative 1-cocycle for w1(TM). This cocycle evaluates to 1
        on edges across which the local orientation flips.

    Algorithm:
        1. Build dual graph: nodes = top simplices, edges = shared (n-1)-faces.
        2. BFS-orient top simplices via a spanning forest of the dual graph.
        3. Identify (n-1)-faces where assigned orientations are inconsistent.
        4. Return a 1-cochain that marks edges in these flip faces.
    """
    n = sc.dimension
    if n < 1:
        return np.zeros(sc.count_simplices(1), dtype=np.int64)

    # 1. Map (n-1)-faces to adjacent n-simplices and induced orientations
    face_to_adj = {}
    top_simplices = [tuple(sorted(s)) for s in sc.n_simplices(n)]

    for i, sigma in enumerate(top_simplices):
        for j in range(n + 1):
            face = sigma[:j] + sigma[j + 1 :]
            if face not in face_to_adj:
                face_to_adj[face] = []
            face_to_adj[face].append((i, j))

    # 2. BFS through dual graph to orient n-simplices
    num_simplices = len(top_simplices)
    orientations = {}  # idx -> epsilon (1 or -1)
    visited = set()
    flip_faces = set()

    for start_node in range(num_simplices):
        if start_node in visited:
            continue

        queue = [(start_node, 1)]
        orientations[start_node] = 1
        visited.add(start_node)

        while queue:
            curr, eps_curr = queue.pop(0)
            sigma_curr = top_simplices[curr]

            for j in range(n + 1):
                face = sigma_curr[:j] + sigma_curr[j + 1 :]
                for neighbor, k in face_to_adj.get(face, []):
                    if neighbor == curr:
                        continue

                    # Compatibility: eps_neighbor = - eps_curr * (-1)^(j+k)
                    needed_eps = -eps_curr * ((-1) ** (j + k))

                    if neighbor not in visited:
                        orientations[neighbor] = needed_eps
                        visited.add(neighbor)
                        queue.append((neighbor, needed_eps))
                    elif orientations[neighbor] != needed_eps:
                        flip_faces.add(face)

    # 3. Build w_1 cochain.
    edge_to_idx = sc.simplex_to_index(1)
    res = np.zeros(sc.count_simplices(1), dtype=np.int64)

    for face in flip_faces:
        if n == 2:
            if face in edge_to_idx:
                res[edge_to_idx[face]] = 1
        else:
            # For higher dims, pick one edge in the flip face as representative
            edge = tuple(sorted(face[:2]))
            if edge in edge_to_idx:
                res[edge_to_idx[edge]] = (res[edge_to_idx[edge]] + 1) % 2
