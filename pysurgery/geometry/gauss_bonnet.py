...
def chern_gauss_bonnet_integral_expected(
    dimension: int, chi: int, *, closed: bool = True
) -> float:
    """Calculate the expected integral of the Euler form.

    What is Being Computed?:
        The theoretical value of the integral of the Euler n-form over a 
        2n-dimensional manifold, which is (2π)ⁿ χ(M). For odd-dimensional 
        closed manifolds, χ must be 0.

    Algorithm:
        1. Check if the dimension is odd.
        2. If odd and closed, verify χ=0 and return 0.0.
        3. If odd and not closed, return NaN if chi != 0 (as the integral depends 
           on boundary terms) or 0.0 if chi == 0.
        4. If even, compute (2π)^(dim/2) * chi.

    Args:
        dimension: The dimension of the manifold.
        chi: The Euler characteristic.
        closed: Whether the manifold is closed (True) or has boundary (False).

    Returns:
        float: The expected integral value.

    Raises:
        MathError: If dimension is odd, closed is True, and chi is non-zero.
    """
    if dimension % 2 != 0:
        if closed and chi != 0:
            raise MathError(
                f"Closed odd-dimensional manifolds must have χ=0; received χ={chi}"
            )
        return float("nan") if chi != 0 else 0.0
