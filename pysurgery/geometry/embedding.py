...
- :class:`~pysurgery.geometry.uniformization.SurfaceMesh`
- :class:`~pysurgery.geometry.geometrization_3d.Triangulated3Manifold`
...
try:  # Optional import to avoid circular/module-load issues.
    from pysurgery.geometry.geometrization_3d import Triangulated3Manifold
...
def check_immersion(pl_map: PLMap, *, tol: float = _EPS, backend: str = "auto") -> ImmersionResult:
    ...
    for vertex in pl_map.vertex_labels:
        ...
        if centered.shape[0] > 1:
            ...
            is_interior = _is_interior_vertex(vertex, pl_map.source_complex)
            expected_rank = top_dim if is_interior else (top_dim - 1)
...
def detect_self_intersections(
    ...
    bboxes = [pl_map.simplex_bounding_box(s) for s in simplices]
    centroids = np.array(
        [pl_map.simplex_barycenter(s) for s in simplices], dtype=np.float64
    )
    ...
    for i, j in sorted(candidate_pairs):
        s1, s2 = simplices[i], simplices[j]
        if simplex_vertex_sets[i].intersection(simplex_vertex_sets[j]):
            pruned += 1
            continue

        if not _bbox_overlap(
            (bbox_lows[i], bbox_highs[i]), (bbox_lows[j], bbox_highs[j]), tol=tol
        ):
            pruned += 1
            continue
        ...
        if center_gap > float(2.0 * tol):
            ...
            continue
        checked += 1
        witness = _simplex_intersection(
            pl_map,
            s1,
            s2,
            tol=tol,
            pa=pl_map.simplex_vertices(s1),
            pb=pl_map.simplex_vertices(s2),
        )
...
def _is_interior_vertex(vertex: int, sc: SimplicialComplex) -> bool:
    """Check if a vertex is in the interior of a homology manifold."""
    n = sc.dimension
    if n <= 1:
        return True

    # Vertex is on boundary if it is in an (n-1)-face that is in only one n-simplex.
    incident_n = [s for s in sc.n_simplices(n) if int(vertex) in s]
    face_counts: dict[tuple[int, ...], int] = {}
    for s in incident_n:
        for j in range(n + 1):
            face = tuple(sorted(s[:j] + s[j + 1 :]))
            if int(vertex) in face:
                face_counts[face] = face_counts.get(face, 0) + 1

    return not any(c == 1 for c in face_counts.values())
