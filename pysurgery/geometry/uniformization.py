...
class SurfaceMesh:
    ...
    def cotangent_laplacian(
        ...
        diag_data = -np.asarray(L_off.sum(axis=1)).ravel()
        L_diag = csr_matrix(
            (
                diag_data,
                (np.arange(self.n_vertices), np.arange(self.n_vertices)),
            ),
...
def _validate_surface(
    ...
    for idx, incident in enumerate(edge_faces):
        if len(incident) > 2:
            raise ValueError(
                f"Edge {tuple(edges[idx])!r} is incident to more than two faces; the input is not a 2-manifold."
            )
    for face in faces:
    ...
    *,
    ...
    for it in range(int(max_iter)):
        ...
        try:
            ...
            notes.append(f"linear solve failed: {exc!r}")
            break
        ...
        if not accepted:
            ...
            if _metric_is_valid(mesh, candidate, method):
                ...
                notes.append(f"line search stalled at iteration {it!r}")
                break
...
def uniformize_surface(
    ...
    *,
    ...
    raise ValueError(f"Unsupported uniformization method: {method!r}")
