...
class SimplicialVectorBundle(BaseModel):
    ...
    def check_cocycle(self) -> bool:
        """Verify the cocycle condition g_uv * g_vw = g_uw on all triangles.
        ...
        Algorithm:
            ...
            2. For each triangle with sorted vertices (u, v, w), fetches matrices g_{uv}, g_{vw}, g_{uw}.
            3. Verifies that the product g_{uv} * g_{vw} is close to g_{uw}.
        ...
        for tri in self.base_complex.n_simplices(2):
            u, v, w = sorted(tri)
            # Standard order: g_uv * g_vw = g_uw
            g_uv = self.transitions.get((u, v))
            g_vw = self.transitions.get((v, w))
            g_uw = self.transitions.get((u, w))

            if g_uv is None or g_vw is None or g_uw is None:
                return False

            if not np.allclose(g_uv @ g_vw, g_uw):
                return False
        return True
