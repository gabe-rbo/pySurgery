...
def coerce_int_matrix(matrix: Any, *, name: str = "matrix") -> np.ndarray:
    ...
    if not np.array_equal(arr_float, rounded):
...
def validate_group_descriptor(descriptor: str) -> tuple[bool, str]:
    ...
    Algorithm:
        ...
        2. Iteratively split and validate product groups separated by "x".
        3. Validate each factor in a single pass to prevent recursion depth issues.
    ...
    if len(factors) > 1:
        for f in factors:
            if f in {"1", "Z", "trivial", "e"}:
                continue
            if f.startswith("Z_"):
                tail = f.split("_", 1)[1]
                if tail.isdigit() and int(tail) > 1:
                    continue
            return False, f"Invalid product factor '{f}'"
        return True, "ok"
    ...
    if d.startswith("Z_"):
        ...
        if tail.isdigit() and int(tail) > 1:
            return True, "ok"
        return False, "Finite cyclic descriptors must be Z_n with n>1"
