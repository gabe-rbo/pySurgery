...
class GroupRingElement:
    ...
    def _parse_cyclic_power(cls, g: str, group_order: int) -> int:
        ...
        Raises:
            GroupRingError: If the generator label is unsupported or not a cyclic power.
        """
        ...
        if gn.startswith("g_"):
            ...
            if not tail.isdigit():
                raise GroupRingError(
                    f"Generator '{g}' is not a cyclic-power label; provide explicit group_law / mul_table."
                )
            return int(tail) % group_order
        if gn.startswith("g") and gn[1:].isdigit():
            return int(gn[1:]) % group_order
        raise GroupRingError(
            f"Generator '{g}' cannot be parsed as a cyclic power; supply group_law / mul_table."
        )
