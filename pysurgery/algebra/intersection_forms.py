...
class IntersectionForm(BaseModel):
    ...
    def signature(self) -> int:
        ...
        Algorithm:
            ...
            2. Perform LDL decomposition to find the diagonal elements (Sylvester's Law of Inertia).
            3. Count positive and negative diagonal entries.
            4. Fallback to exact symbolic eigenvalues if LDL fails or produces invalid results (e.g., indefinite forms or singular matrices without a simple LDL).
            5. Final fallback to numerical eigvalsh only for non-exact reporting paths.
        ...
        import sympy as sp
        M_int = sp.Matrix(self.matrix.astype(int).tolist())
        try:
            # LDL decomposition provides the diagonal D such that M = L D L^T.
            # hermitian=False allows it to work for symmetric (not just positive definite) matrices.
            _, D = M_int.LDLdecomposition(hermitian=False)
            
            # Check for nans which indicate LDL failed to find a valid pivot sequence
            if any(val.has(sp.nan) for val in D):
                raise ValueError("LDL decomposition produced nan; falling back to eigenvalues.")

            pos = sum(1 for i in range(D.rows) if D[i, i] > 0)
            neg = sum(1 for i in range(D.rows) if D[i, i] < 0)
            return int(pos - neg)
        except Exception:
            # Fallback for cases where LDL fails (indefinite with zero leading minors, or singular).
            # Use exact rational eigenvalues for mathematical integrity.
            try:
                rational_eigs = M_int.eigenvals()
                pos = sum(mult for v, mult in rational_eigs.items() if sp.simplify(v) > 0)
                neg = sum(mult for v, mult in rational_eigs.items() if sp.simplify(v) < 0)
                return int(pos - neg)
            except Exception:
                # Absolute fallback to numerical eigenvalues if symbolic fails.
    ...
    def is_hyperbolic(self) -> bool:
        """Check if the form is hyperbolic (stable-isomorphic to a direct sum of hyperbolic planes)."""
        if self.matrix.size == 0:
            return True
        return self.signature() == 0 and self.is_even()
    ...
    def l_group_rational(self, dimension: int, pi: str):
        """Rational L-group obstruction L_*(ℤ) → L_*(ℚ) of this form.

        What is Being Computed?:
            The image of the surgery obstruction under the localisation map
            ``L_*(ℤ) → L_*(ℚ)``: torsion obstructions vanish, and only the
            signature and rank survive. The exact path uses
            :func:`pysurgery.utils.signature.signature_via_snf`; if SNF
            fails it falls back to ``np.linalg.eigvalsh`` and reports
            ``exact=False``.

        Args:
            dimension: Manifold dimension (typically a multiple of 4).
            pi: Fundamental group label.

        Returns:
            :class:`pysurgery.manifolds.rational_surgery.RationalObstruction`.
        """
        from pysurgery.manifolds.rational_surgery import RationalObstruction

        try:
            from ..utils.signature import signature_via_snf
            signature, rank_q = signature_via_snf(self)
            ...
            pos = int(np.sum(eigenvalues > 1e-9))
            neg = int(np.sum(eigenvalues < -1e-9))
            return RationalObstruction(
                dimension=dimension,
                pi=pi,
                signature=pos - neg,
    ...
    def perform_algebraic_surgery(self, x: np.ndarray) -> "IntersectionForm":
        ...
        H = hermite_normal_form(P_sym)
        
        # The non-zero rows of H form the exact Z-basis
        basis_vectors = []
        for i in range(H.rows):
            row = np.array(H[i, :]).astype(int).flatten()
            if np.any(row):
    ...
    def from_complex(
        ...
        *,
        ...
        """Construct Q on H_{n/2}(K; Z) via cup-product. Implements Gap G09.

        Julia acceleration (Acceleration 2):
            When β_m > 50 and Julia is available, the cohomology basis computation
            (SNF of δ_m + exact Gram-matrix inversion via AbstractAlgebra) is
            offloaded to compute_cohomology_basis_jl.  This replaces:
              - smith_normal_decomp (Python/Numba)  →  AbstractAlgebra.snf_with_transform
              - SymPy.Matrix.inv() on the Gram matrix  →  AbstractAlgebra.inv over ℤ
            which is 10–100× faster for β_m > 50.  The cup-product step (step 3)
            already uses Julia threads via compute_alexander_whitney_cup and is
            unchanged.
        """
        import numpy as np
        ...
        if getattr(K, "is_synthetic_e8", False):
            ...
            return cls(matrix=E8, dimension=4)

        from pysurgery.homology.poincare_duality_verification import extract_intersection_pairing_chain

        n = K.dimension
        if n % 2 != 0:
            raise DimensionError("Intersection form requires an even-dimensional complex.")

        m = n // 2

        # ── Fetch boundary matrices (sparse, kept sparse for bridge) ─────────
        dn     = K.boundary_matrix(m)        # ∂_m
        dn_p1  = K.boundary_matrix(m + 1)   # ∂_{m+1}
        dn_top = K.boundary_matrix(n)        # ∂_n
        c_m_size = len(K.n_simplices(m))
        c_n_size = len(K.n_simplices(n))

        # ── Route: Julia for large β_m, Python/SymPy otherwise ───────────────
        _julia_ok = False
        cohom_gens: list = []
        F: np.ndarray = np.zeros(c_n_size, dtype=int)

        use_julia = (backend in ("julia", "auto"))
        if use_julia:
            from pysurgery.bridge.julia_bridge import julia_engine
            if julia_engine.available:
                # Estimate β_m cheaply from rank of dn_p1 to decide routing.
                # If dn_p1 is None / 0-column, β_m = c_m_size − rank(dn).
                # For the threshold check we just look at c_m_size as an upper bound.
                if c_m_size > 50:
                    try:
                        cocycles, nr, nc, free_cols, F_jl = julia_engine.compute_cohomology_basis_jl(
                            dn, dn_p1, dn_top
                        )
                        F = F_jl.astype(int)
                        cohom_gens = [cocycles[:, int(j)] for j in free_cols]
                        _julia_ok = True
                        ...
                        import warnings
                        warnings.warn(
                            f"Julia cohomology basis failed, falling back to Python: {exc!r}"
                        )

        if not _julia_ok:
            # ── Python / SymPy path (unchanged) ─────────────────────────────
            import sympy as sp
            from pysurgery.algebra.math_core import smith_normal_decomp

            if dn is not None and dn.shape[0] > 0 and dn.shape[1] > 0:
                D_m = dn.toarray().astype(int)
            else:
                D_m = np.zeros((0, c_m_size), dtype=int)

            if dn_p1 is not None and dn_p1.shape[0] > 0 and dn_p1.shape[1] > 0:
                D_mp1 = dn_p1.toarray().astype(int)
            else:
                D_mp1 = np.zeros((c_m_size, 0), dtype=int)

            delta_m   = D_mp1.T
            delta_mm1 = D_m.T

            S_m, _, V_m = smith_normal_decomp(delta_m, compute_u=False, compute_v=True)
            r_m = sum(1 for i in range(min(S_m.shape)) if S_m[i, i] != 0)
            Z_m = V_m[:, r_m:]

            if Z_m.shape[1] == 0:
                return cls(matrix=np.zeros((0, 0), dtype=int), dimension=n)

            Z_m_sp     = sp.Matrix(Z_m)
            delta_mm1_sp = sp.Matrix(delta_mm1)
            ZT_Z       = Z_m_sp.T * Z_m_sp
            ZT_Z_inv   = ZT_Z.inv()
            M_sp       = ZT_Z_inv * Z_m_sp.T * delta_mm1_sp
            M          = np.array(M_sp).astype(int)

            S_M, U_M, _ = smith_normal_decomp(M, compute_u=True, compute_v=False)
            U_M_inv     = np.array(sp.Matrix(U_M).inv()).astype(int)
            cocycles    = Z_m @ U_M_inv

            cohom_gens = []
            for j in range(cocycles.shape[1]):
                d_j = int(S_M[j, j]) if j < min(S_M.shape) else 0
                if d_j != 1:
                    cohom_gens.append(cocycles[:, j])

            # Fundamental class F
            if dn_top is not None and dn_top.shape[0] > 0 and dn_top.shape[1] > 0:
                D_n = dn_top.toarray().astype(int)
            else:
                D_n = np.zeros((0, c_n_size), dtype=int)

            S_n, _, V_n = smith_normal_decomp(D_n, compute_u=False, compute_v=True)
            r_n = sum(1 for i in range(min(S_n.shape)) if S_n[i, i] != 0)
            Z_n = V_n[:, r_n:]
            F = Z_n[:, 0] if Z_n.shape[1] > 0 else np.zeros(c_n_size, dtype=int)

        # ── Step 3: cup-product matrix Q (unchanged; already Julia-threaded) ─
        num_gens = len(cohom_gens)
        Q = np.zeros((num_gens, num_gens), dtype=int)
        for i in range(num_gens):
            for j in range(num_gens):
                pairing = extract_intersection_pairing_chain(cohom_gens[i], cohom_gens[j], m, m, K)
                Q[i, j] = int(np.dot(pairing, F))

        if n % 4 == 2:
            Q = np.abs(Q)

        return cls(matrix=Q, dimension=n)
