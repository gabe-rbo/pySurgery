    ...
    def cap_product(
        ...
        if simplices is not None:
            ...
            if c_nk_size is None:
                raise DimensionError(
                    f"Chain group C_{target_dim} not well-defined in underlying complex."
                )
            ...
            if not n_simplices or not k_simplices or not nk_simplices:
                return np.zeros(c_nk_size, dtype=np.int64)
            ...
            for s_idx, coeff in enumerate(self.fundamental_class):
                ...
                # Correcting slice to use disjoint indices as per Alexander-Whitney convention
                front = sigma[: k + 1]
                back = sigma[n - k :]
                ...
                if front in k_map and back in nk_map:
                    ...
                    idx = nk_map[back]
                    result_dict[idx] = result_dict.get(idx, 0) + int(coeff) * int(val)

            result = np.zeros(
                c_nk_size,
                dtype=self.chain_complex.boundaries[1].dtype
                if self.chain_complex.boundaries
                else np.int64,
            )
