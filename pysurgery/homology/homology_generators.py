...
from pysurgery.core.generator_models import HomologyBasisResult, HomologyGenerator
...
_boundary_matrix_cache: dict[tuple, sp.csr_matrix] = {}
...
def _boundary_mod2_matrix(
    ...
    What is Being Computed?:
        The mod-2 boundary matrix d_k mapping k-simplices to (k-1)-simplices.

    Algorithm:
        1. Checks module-level cache for existing matrix.
        2. Delegates to Julia if available for high-performance assembly.
        3. Falls back to pure Python face enumeration.

    Args:
        source: The source simplices (d-simplices).
        target: The target simplices ((d-1)-simplices).
        backend: 'auto', 'julia', or 'python'.

    Returns:
        sp.csr_matrix: The mod-2 boundary matrix.
    """
    if not target or not source:
        return sp.csr_matrix((len(target), len(source)), dtype=np.int64)

    # Use a hashable key for the cache
    key = (tuple(map(tuple, source)), tuple(map(tuple, target)))
    cached = _boundary_matrix_cache.get(key)
    ...
    if use_julia:
        try:
            ...
            _boundary_matrix_cache[key] = result
            return result
    ...
    result = sp.csr_matrix(
        (data, (rows, cols)), shape=(len(target), len(source)), dtype=np.int64
    )
    _boundary_matrix_cache[key] = result
    return result
...
def hk_generators_z(
    K: Union["SimplicialComplex", "ChainComplex"],
    k: int,
    *,
    backend: str = "auto",
) -> List[HomologyGenerator]:
    """ℤ-coefficient generators of H_k(K), with summand labels.
    
    Works on SimplicialComplex or any ChainComplex.
    """
    from pysurgery.algebra.math_core import smith_normal_decomp
    import sympy as sp
    import numpy as np
    
    dn = K.boundary_matrix(k)
    dn_plus_1 = K.boundary_matrix(k + 1)
    
    if hasattr(K, "n_simplices"):
        simplices_k = K.n_simplices(k)
        c_n_size = len(simplices_k)
    else:
        # ChainComplex
        c_n_size = K.boundaries[k].shape[1] if k in K.boundaries else (K.boundaries[k+1].shape[0] if k+1 in K.boundaries else 0)
        # We don't have simplices names, use indices
        simplices_k = [ (i,) for i in range(c_n_size) ]
    
    if dn is not None and dn.shape[0] > 0 and dn.shape[1] > 0:
        D_k = dn.toarray().astype(int)
    else:
        D_k = np.zeros((0, c_n_size), dtype=int)
        
    if dn_plus_1 is not None and dn_plus_1.shape[0] > 0 and dn_plus_1.shape[1] > 0:
        D_k1 = dn_plus_1.toarray().astype(int)
    else:
        D_k1 = np.zeros((c_n_size, 0), dtype=int)
        
    # 1. Compute Smith Normal Decomposition of D_k to get ker(D_k)
    S_k, U_k, V_k = smith_normal_decomp(D_k, compute_u=False, compute_v=True)
    
    # S_k has shape (dim C_{k-1}, dim C_k)
    # The rank r_k is the number of non-zero entries in S_k
    r_k = 0
    for i in range(min(S_k.shape)):
        if S_k[i, i] != 0:
            r_k += 1
            
    # The basis Z of ker(D_k) is given by the columns of V_k from index r_k to the end
    Z = V_k[:, r_k:]
    
    if Z.shape[1] == 0:
        return []
        
    # 2. Express D_{k+1} in terms of the cycle basis Z
    # Solve Z @ M = D_{k+1} exactly using sympy
    Z_sympy = sp.Matrix(Z)
    D_k1_sympy = sp.Matrix(D_k1)
    
    ZT_Z = Z_sympy.T * Z_sympy
    ZT_Z_inv = ZT_Z.inv()
    M_sympy = ZT_Z_inv * Z_sympy.T * D_k1_sympy
    M = np.array(M_sympy).astype(int)
    
    # 3. Compute Smith Normal Decomposition of M
    S_M, U_M, V_M = smith_normal_decomp(M, compute_u=True, compute_v=False)
    
    # Invert U_M exactly
    U_M_sympy = sp.Matrix(U_M)
    U_M_inv_sympy = U_M_sympy.inv()
    U_M_inv = np.array(U_M_inv_sympy).astype(int)
    
    # Representative cycles in C_k are given by Z @ U_M_inv
    cycles = Z @ U_M_inv
    ...
    for j in range(cycles.shape[1]):
        ...
        label = "Z" if d_j == 0 else f"Z/{d_j}"
        
        support_indices = [i for i, val in enumerate(col) if val != 0]
        if not support_indices:
            continue
            
        support_simplices = [simplices_k[i] for i in support_indices]
        cycle_coefficients = {simplices_k[i]: int(col[i]) for i in support_indices}
        
        support_edges = []
        if k == 1:
            support_edges = [tuple(sorted((s[0], s[1]))) for s in support_simplices if len(s) >= 2]
        ...
        gens.append(
            HomologyGenerator(
                ...
                weight=float(len(support_simplices)),
                certified_cycle=True,
                summand_label=label,
            )
        )
