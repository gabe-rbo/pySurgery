...
def _pack_simplices(arr: np.ndarray, n_verts: int) -> np.ndarray:
    """Pack simplex vertex indices into a single int64 for fast hashing/lookup.

    Algorithm:
        Treats the simplex as a base-(n_verts+1) number.
    """
    base = np.int64(n_verts + 1)
    out = np.zeros(arr.shape[0], dtype=np.int64)
    for c in range(arr.shape[1]):
        out = out * base + arr[:, c].astype(np.int64)
    return out
...
def cup_i_product(
    ...
    Algorithm:
        1. Delegates to Julia if available for high-performance interleaving.
        2. Otherwise, generate all valid Steenrod interleaving sequences for a (p+q-i)-simplex.
        3. For each sequence, extract the front and back face indices.
        4. XOR-evaluate the cochains on these faces and sum the results with 
           the appropriate sign determined by the sequence parity.
        5. Uses simplex fingerprinting to avoid expensive tuple/dict overhead in Python.
    ...
    if use_julia:
        try:
            return julia_engine.compute_cup_i_product(
                alpha,
                beta,
                int(p),
                int(q),
            ...
            if backend_norm == "julia":
                raise e
            warnings.warn(f"Julia cup-i fallback failed ({e!r})")

    n = p + q - i
    simplices_arr = np.array(simplices_target, dtype=np.int64)
    result = np.zeros(len(simplices_arr), dtype=np.int64)

    # Determine n_verts for fingerprinting
    n_verts = int(np.max(simplices_arr)) if simplices_arr.size > 0 else 0
    if simplex_to_idx_p:
        v_max_p = max(max(s) for s in simplex_to_idx_p.keys())
        n_verts = max(n_verts, v_max_p)
    if simplex_to_idx_q:
        v_max_q = max(max(s) for s in simplex_to_idx_q.keys())
        n_verts = max(n_verts, v_max_q)

    # Pre-pack cochain dictionaries for fast int64 lookup
    fp_p = _pack_simplices(
        np.asarray(list(simplex_to_idx_p.keys()), dtype=np.int64), n_verts
    )
    idx_p_lookup = {int(fp): idx for fp, idx in zip(fp_p, simplex_to_idx_p.values())}

    fp_q = _pack_simplices(
        np.asarray(list(simplex_to_idx_q.keys()), dtype=np.int64), n_verts
    )
    idx_q_lookup = {int(fp): idx for fp, idx in zip(fp_q, simplex_to_idx_q.values())}
    ...
    # Optimized evaluation using fingerprinting
    for A_idx, B_idx, sgn in valid_seqs:
        fps_a = _pack_simplices(simplices_arr[:, A_idx], n_verts)
        fps_b = _pack_simplices(simplices_arr[:, B_idx], n_verts)

        idx_p = np.fromiter(
            (idx_p_lookup.get(int(f), -1) for f in fps_a),
            dtype=np.int64,
            count=len(fps_a),
        )
        idx_q = np.fromiter(
            (idx_q_lookup.get(int(f), -1) for f in fps_b),
            dtype=np.int64,
            count=len(fps_b),
        )
