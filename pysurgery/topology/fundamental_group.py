...
class FundamentalGroup(BaseModel):
    """Active presentation of the Fundamental Group π₁(X) = ⟨generators | relations⟩.

    Overview:
        FundamentalGroup is an "active mathematical object": it both holds the
        symbolic presentation π₁(X) = ⟨G | R⟩ and exposes group-theoretic
        decision procedures (``is_abelian``, ``is_trivial``, ``order``,
        ``simplify``) plus a geometric bridge (``simplices_generators``) that
        links each abstract generator back to a concrete directed-edge cycle in
        the underlying CW complex.

    Key Concepts:
        - **Presentation (G, R)**: G are generator symbols, R are relator words
          (each a list of tokens) such that π₁(X) ≅ ⟨G | R⟩.
        - **Orientation Character (w₁)**: Map G → {-1, +1} recording which
          generators reverse orientation (non-orientability witness).
        - **Geometric Trace**: Each generator g ∈ G is realised by a directed
          edge cycle in the 1-skeleton of the source complex
          (``Pi1GeneratorTrace``).
        - **Caching**: Expensive group-theoretic invariants
          (abelianization SNF, order via Todd-Coxeter, descriptor inference)
          are memoized for the lifetime of the object.

    Common Workflows:
        1. **From geometry** → ``extract_pi_1(cw)`` returns a
           FundamentalGroup with traces attached.
        2. **Algebraic queries** → ``pi1.is_abelian()`` /
           ``pi1.is_trivial()`` / ``pi1.order()``.
        3. **Reduction** → ``pi1.simplify()`` returns a Tietze-reduced copy.
        4. **Geometric realization** → ``pi1.simplices_generators()``
           returns the directed-edge cycle for every generator.

    Coefficient Ring:
        Pure group theory over ℤ; abelianization SNF is exact.

    Attributes:
        generators: List of generator symbols (strings).
        relations: List of relator words; each is a list of tokens
            (e.g., ['a', 'b', 'a^-1', 'b^-1'] for the commutator).
        orientation_character: Dict[str → {1, -1}] recording w₁(g).
        traces: Optional list of ``Pi1GeneratorTrace`` objects providing the
            geometric realization of each generator (may be empty if the group
            was constructed without spatial data).

    References:
        Tietze, H. (1908). Über die topologischen Invarianten mehrdimensionaler
            Mannigfaltigkeiten. Monatshefte für Math. und Physik 19, 1–118.
        Todd, J. A. & Coxeter, H. S. M. (1936). A practical method for
    ...
    Example:
        pi1 = extract_pi_1(cw)
        if pi1.is_abelian():
            print(pi1)               # < a, b | aba^-1b^-1 >
        edges = pi1.simplices_generators()
        # edges['a'] is e.g. [(0,1), (1,2), (2,0)] — a closed loop.
    """
    ...
    _cache: Dict[tuple, object] = PrivateAttr(default_factory=dict)
    ...
    def _cache_get(self, key: tuple) -> object:
        """Look up a cached invariant; returns sentinel ``_CACHE_MISS`` on miss.

        What is Being Computed?:
            Read-only access to the per-instance memoization dict, used by
            ``is_abelian``, ``is_trivial``, ``order``, etc. so that any
            invariant proven once costs O(1) on subsequent calls.

        Algorithm:
            1. Look up ``key`` in the PrivateAttr-backed cache dict.
            2. Return the stored value, or the module-level ``_CACHE_MISS``
               sentinel if no entry exists (allowing ``None`` to be a legal
               cached value).

        Preserved Invariants:
            - The presentation (generators, relations) is immutable for the
              lifetime of the object, so cached values cannot become stale.

        Args:
            key: Tuple key naming the invariant (e.g. ``("is_abelian",)``).

        Returns:
            The cached value, or ``_CACHE_MISS`` when no entry exists.

        Use When:
            - Internal use by the active-object methods. End users call the
              public methods (``is_abelian()``, ``order()``, ...).

        Example:
            v = self._cache_get(("order",))
            if v is _CACHE_MISS:
                v = self._compute_order_internal()
                self._cache_set(("order",), v)
        """
        return self._cache.get(key, _CACHE_MISS)

    def _cache_set(self, key: tuple, value: object) -> None:
        """Store a computed invariant in the per-instance cache.

        Args:
            key: Tuple key naming the invariant.
            value: The value to memoize.
        """
        self._cache[key] = value

    def clear_cache(self) -> None:
        """Discard all cached invariants. Mainly useful for testing."""
        self._cache.clear()

    def cache_info(self) -> Dict[str, object]:
        """Return a snapshot of the current cache contents (debugging).

        Returns:
            Dict with the number of cached entries and their keys.
        """
        return {
    ...
    def _abelianization_free_rank_torsion(self) -> tuple[int, list[int]]:
        """Return `(free_rank, torsion_invariants)` of the abelianization Z^n / im(M).

        Cached on the instance after the first call.
        """
        cached = self._cache_get(("abelianization",))
        if cached is not _CACHE_MISS:
            return cached  # type: ignore[return-value]
        result = self._compute_abelianization_internal()
        self._cache_set(("abelianization",), result)
        return result

    def _compute_abelianization_internal(self) -> tuple[int, list[int]]:
        """Uncached SNF computation of the abelianization Z^n / im(M).

        Algorithm:
            1. If Julia is available, dispatch to the integer SNF kernel.
            2. Otherwise, build the integer relation matrix and run SymPy's
               Smith normal form.

        Returns:
            (free_rank, sorted nontrivial torsion invariants > 1).
        """
        ...
        if julia_engine.available:
            try:
                free_rank, torsions = julia_engine.abelianize_and_bhs_rank(
                    ...
                    [list(r) for r in self.relations],
                )
                return int(free_rank), [int(t) for t in torsions]
            except Exception:
                pass
        ...
        for rel in self.relations:
            ...
            for tok in rel:
                ...
                if base not in g_to_idx:
                    continue
                row[g_to_idx[base]] += -1 if tok.endswith("^-1") else 1
        ...
        diag = [abs(int(S[i, i])) for i in range(min(S.rows, S.cols))]
        nonzero = [d for d in diag if d != 0]
        torsions = [d for d in nonzero if d > 1]
        free_rank = len(gens) - len(nonzero)
        return free_rank, torsions

    # ── active mathematical methods ───────────────────────────────────────────

    def is_abelian(self) -> bool:
        """Determine whether the group is abelian (commutative).

        What is Being Computed?:
            Decides whether π₁(X), as presented, is commutative — i.e. every
            commutator [g, h] = ghg⁻¹h⁻¹ equals the identity in the group.

        Algorithm:
            1. If 0 or 1 generators remain after Tietze simplification ⇒
               trivially abelian.
            2. Else delegate to ``infer_standard_group_descriptor`` (which
               recognises every finitely-generated abelian group up to
               isomorphism). A non-None descriptor is a *proof* of
               commutativity, since the descriptor language only ranges over
               abelian factor structures (1, ℤ, ℤ_n, products thereof).
            3. Else, attempt a finite-quotient witness for non-abelian: if the
               group order and the abelianization order are both finite and
               unequal, the group is provably non-abelian.
            4. Otherwise raise ``FundamentalGroupError`` — the implementation
               cannot certify the answer (this is undecidable in general).

        Preserved Invariants:
            - Group isomorphism: the answer is a property of the abstract
              group, not of the chosen presentation.
            - Sound by construction: a True result is a proof; a False result
              is a proof; the function never returns a heuristic guess.

        Returns:
            bool: True iff abelian (proven), False iff proven non-abelian.

        Raises:
            FundamentalGroupError: If neither outcome can be certified.

        Use When:
            - Deciding whether π₁ embeds in a cyclic-style classification.
            - Gating Hurewicz/Universal Coefficient simplifications that
              presuppose commutativity.
            - Replacing the older ``infer_standard_group_descriptor(pi1) is
              not None`` idiom in user code.

        Example:
            torus = FundamentalGroup(generators=["a","b"],
                                     relations=[["a","b","a^-1","b^-1"]])
            assert torus.is_abelian() is True
        """
        cached = self._cache_get(("is_abelian",))
        if cached is not _CACHE_MISS:
            return cached  # type: ignore[return-value]

        result = self._compute_is_abelian_internal()
        self._cache_set(("is_abelian",), result)
        return result

    def _compute_is_abelian_internal(self) -> bool:
        """Uncached implementation of ``is_abelian`` (see public method)."""
        # Tietze-simplify first; abelianness is invariant under Tietze moves.
        simp = simplify_presentation(
            list(self.generators), [list(r) for r in self.relations]
        )
        if len(simp.generators) <= 1:
            return True

        # Sound TRUE path: any non-None standard descriptor is necessarily
        # abelian (descriptors range only over 1, ℤ, ℤ_n and products).
        desc = infer_standard_group_descriptor(simp)
        if desc is not None:
            return True

        # Sound FALSE path: finite-order witness vs |abelianization|.
        free_rank, torsions = self._abelianization_free_rank_torsion()
        if free_rank == 0:
            try:
                ord_g = self.order()
        ...
        raise FundamentalGroupError(
            "Cannot certify abelian-ness for this presentation: descriptor "
            "inference inconclusive and no finite-quotient witness was found. "
            "Try simplify() first, or supply a smaller equivalent presentation."
        )

    def is_trivial(self, *, max_index: int = 10_000) -> bool:
        """Decide whether |G| = 1 (the group has only the identity).

        What is Being Computed?:
            Tests whether π₁(X) is the trivial group {1}, equivalently whether
            the universal cover X̃ → X is a homeomorphism.

        Algorithm:
            1. If the generator list is empty after Tietze simplification ⇒ True.
            2. If the abelianization has positive free rank ⇒ False (the
               abelianization, hence G, is infinite).
            3. Otherwise compute ``self.order(max_index=max_index)``; the
               group is trivial iff that order equals 1. (Uses the Julia
               Todd-Coxeter backend when available.)

        Preserved Invariants:
            - Trivial-or-not is a homotopy invariant of X.
            - True is a proof; False is a proof; ``FundamentalGroupError``
              indicates the question could not be answered within the budget.

        Args:
            max_index: Coset-enumeration ceiling forwarded to ``order``.

        Returns:
            bool: True iff G is the trivial group.

        Raises:
            FundamentalGroupError: When Todd-Coxeter exceeds ``max_index``
                and finiteness cannot be settled.

        Use When:
            - Detecting simply-connected manifolds prior to higher-π_n work.
            - Validating that a 2-skeleton presentation collapses to the disk.

        Example:
            disc = FundamentalGroup(generators=["a"], relations=[["a"]])
            assert disc.is_trivial() is True
        """
        cached = self._cache_get(("is_trivial", int(max_index)))
        if cached is not _CACHE_MISS:
            return cached  # type: ignore[return-value]

        if not self.generators:
            self._cache_set(("is_trivial", int(max_index)), True)
            return True

        # If abelianization is infinite, group is infinite, so non-trivial.
        free_rank, torsions = self._abelianization_free_rank_torsion()
        if free_rank > 0:
            self._cache_set(("is_trivial", int(max_index)), False)
            return False
        # If abelianization is non-trivial finite, group is non-trivial.
        if any(t > 1 for t in torsions):
            self._cache_set(("is_trivial", int(max_index)), False)
            return False

        # Otherwise, run Todd-Coxeter via order().
        result = self.order(max_index=max_index) == 1
        self._cache_set(("is_trivial", int(max_index)), result)
        return result

    def simplify(self) -> "FundamentalGroup":
        """Return a Tietze-simplified copy of this presentation.

        What is Being Computed?:
            Applies the deterministic Tietze-lite reduction implemented in
            ``simplify_presentation`` and returns a new FundamentalGroup
            object on the reduced generator/relation list. Geometric traces
            for surviving generators are propagated to the new object.

        Algorithm:
            1. Call ``simplify_presentation(self.generators, self.relations)``.
            2. Filter ``self.traces`` to retain only those whose generator
               name survived simplification.
            3. Project ``orientation_character`` onto the surviving generators.

        Preserved Invariants:
            - Group isomorphism: the simplified presentation defines an
              isomorphic abstract group.
            - All derived invariants (abelianization, order, is_abelian) are
              identical between the two FundamentalGroup objects.

        Returns:
            FundamentalGroup: An equivalent presentation with (typically)
            fewer generators and shorter relators.

        Use When:
            - Before printing or hashing a presentation.
            - Before group-ring construction; smaller G ⇒ smaller ℤ[G].
            - Whenever ``is_abelian`` / ``order`` / descriptor inference
              is inconclusive on the raw presentation.

        Example:
            small = pi1.simplify()
            print(small)  # may collapse < a, b | a > to < b | >
        """
        simp = simplify_presentation(
            list(self.generators), [list(r) for r in self.relations]
        )
        keep = set(simp.generators)
        new_w1 = {g: self.orientation_character.get(g, 1) for g in simp.generators}
        new_traces = [tr for tr in self.traces if tr.generator in keep]
        return FundamentalGroup(
            generators=list(simp.generators),
            relations=[list(r) for r in simp.relations],
            orientation_character=new_w1,
            traces=new_traces,
        )

    def simplices_generators(self) -> Dict[str, List[Tuple[int, int]]]:
        """Geometric realization: directed-edge cycle for each generator.

        What is Being Computed?:
            For each abstract group generator g ∈ G, returns the directed-edge
            sequence in the 1-skeleton of the source CW complex that realises
            g as a closed loop based at the chosen spanning-tree root. This is
            the bridge from the algebraic π₁ presentation back to concrete
            simplices in the underlying space.

        Algorithm:
            1. Look up the ``Pi1GeneratorTrace`` whose ``generator`` field
               matches each name in ``self.generators``.
            2. Read off ``trace.directed_edge_path``: a list of (u, v) edges.
            3. Build the dictionary ``{generator_name → [(u, v), ...]}``.
            4. Generators without a trace map to an empty list.

        Preserved Invariants:
            - **Closed cycle**: For a non-empty edge path produced by a CW
              extraction, ``path[0][0] == path[-1][1]`` (loop closes at the
              spanning-tree root or the generator's source vertex).
            - The vertex/edge indices reference the *original* CW or
              SimplicialComplex from which π₁ was extracted; they are stable
              under Tietze simplification only for surviving generators.

        Returns:
            Dict mapping each generator name to a list of directed edges
            ``[(u, v), …]`` in the underlying complex.

        Use When:
            - Visualising π₁ generators on the source mesh / CW skeleton.
            - Pulling back symbolic group elements to geometric cycles
              (e.g., for surgery-theoretic transverse-intersection arguments).
            - Building a geometric chain representative for the Hurewicz
              image H₁(X; ℤ).

        Example:
            edges = pi1.simplices_generators()
            for gen, path in edges.items():
                print(gen, "→", path)
            # 'a' → [(0, 1), (1, 2), (2, 0)]  ← a closed loop
        """
        cached = self._cache_get(("simplices_generators",))
        if cached is not _CACHE_MISS:
            return cached  # type: ignore[return-value]

        out: Dict[str, List[Tuple[int, int]]] = {g: [] for g in self.generators}
        for tr in self.traces:
            if tr.generator in out:
                out[tr.generator] = [
                    (int(u), int(v)) for (u, v) in tr.directed_edge_path
                ]
        self._cache_set(("simplices_generators",), out)
        return out

    def is_finite(self, *, max_index: int = 10_000) -> bool:
        """Determine whether the group is finite.

        What is Being Computed?:
            Decides whether |⟨generators | relations⟩| is finite. Returns False
            for any group with infinite abelianization. For groups with finite
            abelianization, runs Todd-Coxeter coset enumeration over the
            trivial subgroup with a bound `max_index`; raises
            `FundamentalGroupError` if the bound is exceeded (i.e. either the
            group is infinite or the procedure could not certify finiteness
            within the budget).

        Algorithm:
            1. Empty generator list ⇒ trivial group ⇒ finite.
            2. Compute abelianization SNF; if any free rank > 0, the
               abelianization (and hence the group) has infinite order.
            3. Otherwise dispatch to `todd_coxeter_index_jl` in the Julia
               backend (or to a Python fallback for the trivial / cyclic cases
               that are decidable without coset enumeration).

        Preserved Invariants:
            Decidability under bound: a `True` result is a proof of finiteness;
            `False` is a proof of infinitude (via abelianization);
            `FundamentalGroupError` indicates the call cannot be answered
            within `max_index`.

        Args:
            max_index: Maximum number of cosets to enumerate before declaring
                the procedure undecidable.

        Returns:
            bool: True iff the group is finite and was certified within the bound.

        Raises:
            FundamentalGroupError: If the abelianization is finite but
                Todd-Coxeter exceeded `max_index` cosets without converging.

        Use When:
            - Gating a `UniversalCover` construction to finite π₁.
            - Validating that a `FiniteGroupRing` can be instantiated.

        Example:
            pi1 = FundamentalGroup(generators=["a"], relations=[["a", "a"]])
            assert pi1.is_finite() is True  # π₁(RP²) = ℤ/2

        References:
            Todd, J. A. & Coxeter, H. S. M. (1936). A practical method for
              enumerating cosets of a finite abstract group. Proceedings of
              the Edinburgh Mathematical Society, 5, 26–34.
        """
        cached = self._cache_get(("is_finite", int(max_index)))
        if cached is not _CACHE_MISS:
            if isinstance(cached, FundamentalGroupError):
                raise cached
            return cached  # type: ignore[return-value]
        try:
            result = self._compute_is_finite_internal(max_index=max_index)
        except FundamentalGroupError as exc:
            self._cache_set(("is_finite", int(max_index)), exc)
            raise
        self._cache_set(("is_finite", int(max_index)), result)
        return result

    def _compute_is_finite_internal(self, *, max_index: int = 10_000) -> bool:
        """Uncached implementation of ``is_finite``."""
        if not self.generators:
            return True
        free_rank, _ = self._abelianization_free_rank_torsion()
        if free_rank > 0:
            return False
        if julia_engine.available:
            try:
                flat_rels = [" ".join(r) for r in self.relations]
                converged, _n_cosets, _table = julia_engine.todd_coxeter_index(
                    list(self.generators), flat_rels, int(max_index)
                )
                if converged:
                    return True
                raise FundamentalGroupError(
                    f"Todd-Coxeter exceeded max_index={max_index}; group is "
                    "infinite or its finiteness is undecidable within the bound."
                )
            except FundamentalGroupError:
                raise
            except Exception as exc:
                raise FundamentalGroupError(
                    f"Todd-Coxeter coset enumeration failed: {exc!r}"
                ) from exc
        if len(self.generators) == 1 and self.relations:
            g = self.generators[0]
            for rel in self.relations:
                exp = sum(-1 if t.endswith("^-1") else 1 for t in rel
                          if (t[:-3] if t.endswith("^-1") else t) == g)
                if exp != 0:
                    return True
        raise FundamentalGroupError(
            "Cannot certify finiteness without the Julia backend; install "
            "juliacall and AbstractAlgebra, or supply a presentation whose "
            "finiteness is decidable from abelianization alone."
        )

    def order(self, *, max_index: int = 10_000) -> int:
        """Compute |G| for a finite group, raising for infinite groups.

        What is Being Computed?:
            The cardinality |G| of the group ⟨generators | relations⟩.
            Returned only when |G| is finite *and* certifiable within
            ``max_index`` cosets.

        Algorithm:
            1. Empty generators ⇒ |G| = 1.
            2. If the abelianization has positive free rank ⇒ |G| = ∞ ⇒ raise.
            3. Run Todd-Coxeter coset enumeration over the trivial subgroup
               via the Julia backend; the converged index equals |G|.

        Preserved Invariants:
            - The returned integer is exact (Todd-Coxeter is an exact
              decision procedure when it converges).
            - Result is memoized; repeated calls cost O(1).

        Args:
            max_index: Upper bound on coset table size before declaring the
                procedure undecidable.

        Returns:
            int: |G|.

        Raises:
            FundamentalGroupError: If G is infinite, or finite but not
                certifiable within the budget, or the Julia backend is
                unavailable for a presentation that requires it.

        Use When:
            - Validating that |G| matches an expected manifold invariant
              (e.g., π₁(L(p,q)) of order p).
            - Sizing a finite group ring before iterating over it.

        Example:
            rp2 = FundamentalGroup(generators=['a'], relations=[['a','a']])
            assert rp2.order() == 2
        """
        cached = self._cache_get(("order", int(max_index)))
        if cached is not _CACHE_MISS:
            if isinstance(cached, FundamentalGroupError):
                raise cached
            return cached  # type: ignore[return-value]
        try:
            result = self._compute_order_internal(max_index=max_index)
        except FundamentalGroupError as exc:
            self._cache_set(("order", int(max_index)), exc)
            raise
        self._cache_set(("order", int(max_index)), result)
        return result

    def _compute_order_internal(self, *, max_index: int = 10_000) -> int:
        """Uncached implementation of ``order``."""
        if not self.generators:
            return 1
        free_rank, torsions = self._abelianization_free_rank_torsion()
        if free_rank > 0:
            raise FundamentalGroupError(
                "Cannot compute order: group is infinite (abelianization has "
                f"free rank {free_rank})."
            )
        if julia_engine.available:
            try:
                flat_rels = [" ".join(r) for r in self.relations]
                converged, n_cosets, _table = julia_engine.todd_coxeter_index(
                    list(self.generators), flat_rels, int(max_index)
                )
                if converged:
                    return int(n_cosets)
                raise FundamentalGroupError(
                    f"Todd-Coxeter exceeded max_index={max_index}; cannot "
                ...
                raise FundamentalGroupError(
                    f"Todd-Coxeter failed: {exc!r}"
                ) from exc
        if len(self.generators) == 1 and torsions and not free_rank:
            return int(torsions[0])
        raise FundamentalGroupError(
            "Cannot compute order without the Julia backend for general "
            "presentations."
        )
...
def _pi1_raw_data_python(cw: CWComplex, backend: str = "auto"):
    ...
    for edge_idx, gen_name in raw_gen_map.items():
        ...
        path_v_u = _path_between_tree(v, u, parent, depth)
        path = [u] + path_v_u
        directed = [(path[i], path[i+1]) for i in range(len(path)-1)]
