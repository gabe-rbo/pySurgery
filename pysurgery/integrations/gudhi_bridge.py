...
def _extract_complex_data_python(simplex_tree, simplices=None, max_dim=None):
    ...
    simplex_to_idx = {
        k: {s: i for i, s in enumerate(dim_simplices[k])} for k in dim_simplices
    }
    ...
    for k in range(1, max_dim + 1):
        ...
        boundaries[k] = sp.csr_matrix(
            (data, (rows, cols)), shape=(n_rows, n_cols), dtype=np.int64
        )
...
def extract_complex_data(simplex_tree, *, include_metadata: bool = True, backend: str = "auto"):
    ...
    if use_julia:
        try:
            ...
            if include_metadata:
                boundary_payload, cells, dim_simplices, simplex_to_idx = (
                ...
                boundary_payload, cells = (
                    julia_engine.compute_boundary_payload_from_simplices(
                        simplex_entries,
                        max_dim,
...
def simplex_tree_to_intersection_form(
    ...
    if 4 in boundaries:
        ...
        if use_julia:
            try:
                ...
                basis_4 = julia_engine.compute_sparse_cohomology_basis(
                    boundaries[4].T, None, cn_size=cells[4]
                )
                if len(basis_4) > 0:
        ...
        if not fund_class_found:
            ...
            try:
                if cells[4] == 1:
                    ...
                    tol = (
                        cells[4] * np.finfo(float).eps * max(s) if len(s) > 0 else 1e-10
                    )
        ...
        if not fund_class_found:
            raise HomologyError(
                "No fundamental class [M] found (H_4 is empty or computation failed). "
        ...
        raise HomologyError(
            "Fundamental class requires topological boundary data. Cannot default to vector of ones."
        )
    ...
    if not np.allclose(Q, Q.T, atol=1e-8):
        raise HomologyError(
            "Cup product matrix is not symmetric — indicates a basis computation failure. Forcing symmetry may corrupt the intersection form."
        )
...
def signature_landscape(
    ...
    if simplex_tree.dimension() != 4:
        ...
        warnings.warn(
            "signature_landscape currently hard-codes dimension 4. Other dimensions will return signature 0."
        )
    ...
    for val in filtration_values:
        ...
        try:
            q_form = simplex_tree_to_intersection_form(
                st_sub, allow_approx=allow_approx
            )
...
def triangulate_surface_python(
    ...
    projected_2d = np.column_stack([centered @ v1, centered @ v2])
