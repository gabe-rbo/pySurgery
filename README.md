...
Most numerical linear-algebra approaches over ℝ lose information that lives over **ℤ** (especially torsion). pySurgery focuses on *exact* and *structure-aware* invariants:

- **Homology & cohomology over ℤ** via **Smith Normal Form (SNF)** (captures torsion)
...
pySurgery can offload some exact integer workloads to Julia (recommended for very large sparse exact-ℤ computations).
...
7. `07_fundamental_group_and_structure_set.ipynb` — π₁ extraction + structure-set oriented workflows
...
### 1) Homology & cohomology via Smith Normal Form (over ℤ)

When computing homology
\[
H_n(X)=\ker(d_n) / \operatorname{im}(d_{n+1}),
\]
floating-point linear algebra misses **torsion**. pySurgery implements **Smith Normal Form** over **ℤ** to recover:
- Betti ranks (free part)
- torsion coefficients (e.g. ℤₖ factors)
...
In dimensions \(n \ge 5\), classification interacts with π₁, Whitehead torsion, and Wall’s surgery groups \(L_n(\pi_1)\). pySurgery includes tooling aimed at discrete extraction of group presentations and surgery-sequence-adjacent invariants (where computationally feasible).
...
- **5D+**: surgery-theoretic invariants and π₁-sensitive obstructions
...
3. **Julia bridge** for large exact-ℤ workloads when Python becomes the bottleneck
...
- performance improvements and additional exact-ℤ backends
...
- Non-abelian / twisted coefficient systems (π₁-sensitive homology)
