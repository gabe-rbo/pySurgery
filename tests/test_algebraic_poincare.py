...
def test_cap_product():
    ...
    apc = AlgebraicPoincareComplex(
        chain_complex=cc, fundamental_class=fund_class, dimension=1, psi={0: psi_0}
    )
...
def test_psi_default_is_not_shared_between_instances():
    ...
    a = AlgebraicPoincareComplex(
        chain_complex=cc, fundamental_class=np.array([1]), dimension=0
    )
    b = AlgebraicPoincareComplex(
        chain_complex=cc, fundamental_class=np.array([1]), dimension=0
    )
...
def test_fundamental_class_length_mismatch_raises():
    ...
    with pytest.raises(DimensionError):
        AlgebraicPoincareComplex(
            chain_complex=cc, fundamental_class=np.array([1]), dimension=2
        )
