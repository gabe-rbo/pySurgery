"""
Tests for pysurgery/core/handle_surgery.py
...
# ── Test 6: perform_surgery raises SurgeryPostconditionError on bad complex ───


def test_perform_surgery_raises_on_invalid_attachment(circle_s1):
    """perform_surgery raises HandleSurgeryError when attaching sphere not in K.
    ...
    """
    from pysurgery.manifolds.handle_surgery import HandleAttachment
    ...
    with pytest.raises((AttachmentSphereError, HandleSurgeryError)):
        perform_surgery(K, attachment, backend="python")
...
# ── Test 10: perform_surgery on non-manifold raises error ─────────────────────


def test_perform_surgery_non_manifold():
    """perform_surgery on a non-manifold complex raises AttachmentSphereError or HandleSurgeryError.
    ...
    try:
        result = perform_surgery(K, attachment, backend="python")
