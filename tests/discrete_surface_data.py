...
def to_complex(st):
    ...
    return ChainComplex(boundaries=boundaries, dimensions=list(cells.keys()), cells=cells)
...
def get_surfaces():
    ...
    for nx in [3, 4, 5]:
        for ny in [3, 4, 5]:
            surfaces.append((f"Torus_{nx}x{ny}", lambda nx=nx, ny=ny: build_torus_param(nx, ny), {0: 1, 1: 2, 2: 1}, {1: []}, 0))
    return surfaces
